...
export default function AIToolsPage() {
    ...
    initialMessages: [
      {
        ...
        role: 'assistant',
        content:
          "Hello! I'm Abyss AI, your intelligent assistant for the Abyss Protocol. I can help you with vault management, security analysis, and portfolio insights. What would you like to know?",
      },
    ],
  });
  const { messages, sendMessage, status } = chatInstance;
  const isLoading = status === 'streaming' || status === 'submitted' || status === 'generating';
  ...
  return (
    <div className="min-h-screen bg-[#040810] relative w-full flex flex-col overflow-x-hidden">
      ...
      {/* ═══ AI CHAT SECTION ═══ */}
      <section className="relative z-10 w-full px-6 md:px-12 lg:px-20 py-10 pb-20">
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-6">
              <div
                className="w-10 h-10 rounded-xl flex items-center justify-center border border-[rgba(124,58,237,0.3)]"
                style={{
                  background: 'rgba(124,58,237,0.1)',
                  boxShadow: '0 0 20px rgba(124,58,237,0.15)',
                }}
              ...
              <div>
                <h2 className="text-xl font-bold text-[#e8f0fe]">Abyss AI Chat</h2>
                <p className="text-xs text-[#6b82a8] font-medium">Ask anything about the Abyss Protocol</p>
              </div>
              <div className="ml-auto flex items-center gap-2 px-3 py-1.5 rounded-full bg-[rgba(0,245,160,0.08)] border border-[rgba(0,245,160,0.2)]">
                ...
                <span className="text-[#00f5a0] text-xs font-bold uppercase tracking-wider">Online</span>
              </div>
            </div>

            {/* Chat container */}
            <div
              className="rounded-2xl border border-[rgba(0,212,255,0.12)] overflow-hidden"
              style={{
                background: 'rgba(8,15,30,0.6)',
                backdropFilter: 'blur(20px)',
              }}
            >
              ...
              <div className="flex items-center gap-2 px-5 py-3 border-b border-[rgba(0,212,255,0.1)] bg-[rgba(0,212,255,0.03)]">
                ...
                <span className="font-mono text-xs text-[#6b82a8] font-bold ml-2">ABYSS_AI_CHAT.SYS</span>
              </div>
              ...
              >
                      ...
                      <div className="prose-chat">
                        {msg.content ? (
                          <ReactMarkdown
                            ...
                            components={{
                              ...
                              h2: ({node, ...props}) => <h2 className="text-base font-bold mb-2 mt-3 text-[#00d4ff]" {...props} />,
                              h3: ({node, ...props}) => <h3 className="text-sm font-bold mb-1 mt-2 text-[#7c3aed]" {...props} />,
                              ul: ({node, ...props}) => <ul className="list-disc pl-5 mb-2 space-y-1" {...props} />,
                              ol: ({node, ...props}) => <ol className="list-decimal pl-5 mb-2 space-y-1" {...props} />,
                              li: ({node, ...props}) => <li className="pl-1" {...props} />,
                              strong: ({node, ...props}) => <strong className="font-bold text-[#e8f0fe]" {...props} />,
                              table: ({node, ...props}) => <div className="overflow-x-auto mb-3"><table className="w-full text-left border-collapse" {...props} /></div>,
                              th: ({node, ...props}) => <th className="border-b border-[rgba(0,212,255,0.2)] py-2 px-3 font-bold text-[#6b82a8] text-xs uppercase tracking-widest" {...props} />,
                              td: ({node, ...props}) => <td className="border-b border-[rgba(0,212,255,0.05)] py-2 px-3" {...props} />,
                              code: ({node, inline, ...props}: any) => (
                                inline 
                                ? <code className="bg-[rgba(0,0,0,0.3)] px-1.5 py-0.5 rounded text-[#00f5a0] font-mono text-xs" {...props} />
                                : <code className="block bg-[rgba(0,0,0,0.4)] p-3 rounded-lg text-[#00f5a0] my-2 overflow-x-auto whitespace-pre-wrap font-mono text-xs border border-[rgba(0,212,255,0.1)]" {...props} />
                              ),
                            }}
                          >
                            {msg.content}
                        ...
                        {msg.toolInvocations?.map((toolInvocation) => {
                          ...
                          if (!('result' in toolInvocation)) {
                            return (
                              <div key={toolCallId} className="flex items-center gap-2 text-xs text-[#00d4ff] mt-2 mb-1 p-2 rounded-lg bg-[rgba(0,212,255,0.05)] border border-[rgba(0,212,255,0.1)]">
                          ...
                          return (
                            <div key={toolCallId} className="flex items-center gap-2 text-xs text-[#00f5a0] mt-2 mb-1 p-2 rounded-lg bg-[rgba(0,245,160,0.05)] border border-[rgba(0,245,160,0.1)]">
                              <CheckCircle2 className="w-3.5 h-3.5" />
                              <span>Completed {toolInvocation.toolName}</span>
                            </div>
                          );
                        })}
                      </div>
                ...
                {isLoading && messages.length > 0 && messages[messages.length - 1].role === 'user' && (
                  <div className="flex justify-start">
                    <div
                      className="rounded-2xl rounded-bl-sm px-4 py-3 inline-flex items-center gap-1.5"
                      style={{
                        background: 'rgba(8,15,30,0.8)',
                        border: '1px solid rgba(0,212,255,0.12)',
                      }}
                    >
                      <span
                        className="w-2 h-2 rounded-full bg-[#00d4ff] opacity-60"
                        style={{ animation: 'pulse-glow 1s ease-in-out infinite' }}
                      />
                      <span
                        className="w-2 h-2 rounded-full bg-[#00d4ff] opacity-60"
                        style={{ animation: 'pulse-glow 1s ease-in-out 0.2s infinite' }}
                      />
                      <span
                        className="w-2 h-2 rounded-full bg-[#00d4ff] opacity-60"
                        style={{ animation: 'pulse-glow 1s ease-in-out 0.4s infinite' }}
                      />
                    </div>
                  </div>
                )}
              ...
              {/* Input area */}
              <div className="border-t border-[rgba(0,212,255,0.1)] p-4 bg-[rgba(4,8,16,0.5)]">
                <form onSubmit={onChatSubmit} className="flex gap-3">
                  <input
                    id="chat-input"
                    type="text"
                    placeholder="Ask Abyss AI anything..."
                    value={input}
                    onChange={(e) => setInput(e.target.value)}
                    className="flex-1 px-4 py-3 rounded-xl text-sm text-[#e8f0fe] placeholder-[#6b82a8] outline-none transition-all duration-300 focus:border-[#7c3aed]"
                    style={{
                      background: 'rgba(8,15,30,0.8)',
                      border: '1px solid rgba(0,212,255,0.15)',
                    }}
                  ...
                  <button
                    ...
                    className="btn-primary w-12 h-12 rounded-xl flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0"
                  >
                    {isLoading ? (
